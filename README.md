...
The test is divided into three parts, a refactoring part, a cache integration part and a cache implementation part.
- The refactoring part is meant to test your ability to refactor code and apply clean code principles. 
- The cache integration part is meant to test your ability to integrate the cache into the data layer.
- The cache implementation part is meant to test your ability to implement the cache functionality.
...
## Task 3 - Cache Implementation (30 min)
Implement the LRU cache functionality by completing the `get`, `set`, and `has` methods in the `lru-cache.ts` file. Your implementation should handle both the time-to-live (TTL) constraints and item limits while maintaining proper least-recently-used eviction behavior.
You can check your implementation against the `lru-cache.test.ts` file by running `npm run test`.
